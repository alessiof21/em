    ...
    export default {
        ...
        methods: {
            createUser() { // Создание пользователя, отправка формы
                ...
                axios.post('/', values)
                    .then(() => {
                        //Если у нас произошло добавление пользователя и мы это передали в базу данных, то сообщаем, что база данных изменилась, нужно заново к ней обращаться
            ...
            saveChanges(id) {
                ...
                if (this.cache[id].firstName !== firstName || this.cache[id].nickName !== nickName) {
                    ...
                    axios.post('/editUsers', editUser)
                        .then(() => {
                            //Если у нас произошло изменение пользователя и мы это передали в базу данных, то сообщаем, что база данных изменилась
                            this.$emit('changeTrue');
                        })
                        .catch(e => console.error(e.message))
                }
